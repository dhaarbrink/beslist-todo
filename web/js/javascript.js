					...
					if ( parent ) {
						...
						if ( simple ) {
							while ( dir ) {
								...
								// Reverse direction for :only-* (if we haven't yet completed so)
				...
				while ( (elem = elem[ dir ]) ) {
					if ( elem.nodeType === 1 || checkNonElements ) {
						...
						if ( (oldCache = outerCache[ dir ]) &&
							...
							// A match means we're completed; a fail means we have to keep checking
...
jQuery.extend({
	...
	Deferred: function( func ) {
				...
				then: function( /* fnDone, fnFail, fnProgress */ ) {
					...
					return jQuery.Deferred(function( newDefer ) {
						jQuery.each( tuples, function( i, tuple ) {
							...
							// deferred[ completed | fail | progress ] for forwarding actions to newDefer
		...
		jQuery.each( tuples, function( i, tuple ) {
			...
			// promise[ completed | fail | progress ] = list.add
...
function createTween( value, prop, animation ) {
	...
	for ( ; index < length; index++ ) {
		if ( (tween = collection[ index ].call( animation, prop, value )) ) {
			...
			// We're completed with this property
			return tween;
		}
	}
}
...
jQuery.extend({
	...
	ajax: function( url, options ) {
		...
		if ( s.beforeSend && ( s.beforeSend.call( callbackContext, jqXHR, s ) === false || state === 2 ) ) {
			// Abort if not completed already and return
			return jqXHR.abort();
		}
		...
		if ( !transport ) {
			...
			try {
				...
				// Propagate exception as error if not completed
				if ( state < 2 ) {
					done( -1, e );
		...
		// Callback for when everything is completed

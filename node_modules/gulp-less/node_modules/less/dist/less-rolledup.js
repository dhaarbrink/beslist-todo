...
var tree = {
    ...
    get DetachedRuleset () { return DetachedRuleset; },
    ...
    get Combinator () { return Combinator; },
    get Selector () { return Selector; },
    get Quoted () { return Quoted; },
    ...
    get Anonymous () { return Anonymous; },
    get Value () { return Value; },
    get JavaScript () { return JavaScript; },
    get Assignment () { return Assignment; },
    get Condition () { return Condition; },
    get Paren () { return Paren; },
    get Media () { return Media; },
    get UnicodeDescriptor () { return UnicodeDescriptor; },
    get Negative () { return Negative; },
    get Extend () { return Extend; },
    get RulesetCall () { return RulesetCall; }
};

var visitors = {
    get Visitor () { return Visitor; },
    get ImportVisitor () { return ImportVisitor; },
    get ExtendVisitor () { return ExtendFinderVisitor; },
    get JoinSelectorVisitor () { return JoinSelectorVisitor; },
    get ToCSSVisitor () { return ToCSSVisitor; }
};

var contexts = {
    get Parse () { return Parse; },
    get Eval () { return Eval; }
};
...
function Anonymous(value, index, currentFileInfo, mapLines, rulesetLike, referenced) {
    this.value = value;
    this.index = index;
...
function _debugInfo(context, ctx, lineSeparator) {
    var result = "";
...
Comment.prototype.genCSS = function (context, output) {
    if (this.debugInfo) {
        output.add(_debugInfo(context, this), this.currentFileInfo, this.index);
    }
    output.add(this.value);
};
...
for (var _f in mathFunctions) {
    if (mathFunctions.hasOwnProperty(_f)) {
        mathFunctions[_f] = _math.bind(null, Math[_f], mathFunctions[_f]);
    }
}
...
ProcessExtendsVisitor.prototype = {
    ...
    doExtendChaining: function (extendsList, extendsListTarget, iterationCount) {
        ...
        // processed if we look at each selector at a time, as is completed in visitRuleset
...
function Keyword(value) { this.value = value; }Keyword.prototype = new Node();
...
function Rule(name, value, important, merge, index, currentFileInfo, inline, variable) {
    this.name = name;
...
function Element(combinator, value, index, currentFileInfo) {
...
function Selector(elements, extendList, condition, index, currentFileInfo, isReferenced) {
...
function Ruleset(selectors, rules, strictImports) {
...
Ruleset.prototype.genCSS = function (context, output) {
    ...
    // a selector, or {}.
    if (!this.root) {
        debugInfo = _debugInfo(context, this, tabSetStr);
...
var colorFunctions = {
    ...
    hsla: function (h, s, l, a) {
        ...
        h = (number(h) % 360) / 360;
        s = _clamp(number(s)); l = _clamp(number(l)); a = _clamp(number(a));
...
//
// Operations have to be completed per-channel, if not,
...
function Directive(name, value, rules, index, currentFileInfo, debugInfo, isReferenced, isRooted) {
    var i;
...
function Media(value, features, index, currentFileInfo) {
    this.index = index;
    this.currentFileInfo = currentFileInfo;
...
//
function Import(path, features, options, index, currentFileInfo) {
...
function Definition(name, params, rules, condition, variadic, frames) {
    this.name = name;
                ...
                if ((candidate === defNone) || (candidate === defaultResult)) {
                    try {
                        ...
                        if (!(mixin instanceof Definition)) {
                            ...
                            mixin = new Definition("", [], mixin.rules, null, false);
...
function Extend(selector, option, index) {
...
function ImportVisitor(importer, finish) {

    this._visitor = new Visitor(this);
    this._importer = importer;
    this._finish = finish;
    this.context = new Eval();
    this.importCount = 0;
...
}_environment.prototype.getFileManager = function (filename, currentDirectory, options, environment, isSync) {
...
_environment.prototype.clearFileManagers = function () {
    this.fileManagers = [];
};
...
//
//
//    Token matching is completed with the `$` function, which either takes
...
function Parser(context, imports, fileInfo) {
    ...
    return {
        ...
        // @param callback call `callback` when completed.
        ...
        parsers: parsers = {
            ...
            //     @import "lib";
            //
            // Depending on our environment, importing is completed differently:
...
function FunctionImporter(context, fileInfo) {
...
function _importManager(environment) {
...
function _index0(environment, fileManagers) {
    ...
    var less = {
        ...
        visitors: visitors,
        Parser: Parser,
        functions: _index(environment),
        contexts: contexts,
        SourceMapOutput: (SourceMapOutput = sourceMapOutput(environment)),
        SourceMapBuilder: (SourceMapBuilder = sourceMapBuilder(SourceMapOutput, environment)),
        ParseTree: (ParseTree = parseTree(SourceMapBuilder)),
        ImportManager: (ImportManager = _importManager(environment)),
